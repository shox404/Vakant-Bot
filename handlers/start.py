...
async def add_user(message: Message):
    ...
    if str(user_id) in ADMINS:
        ...
        if current_time.hour < 12:
            await message.answer(f"Good morning. Welcome to the bot ADMIN <b>{message.from_user.full_name}</b>! ")
        elif current_time.hour < 18 and current_time>12:
           await message.answer(f"Good afternoon. Welcome to the bot ADMIN <b>{message.from_user.full_name}</b>! ")
        else:
